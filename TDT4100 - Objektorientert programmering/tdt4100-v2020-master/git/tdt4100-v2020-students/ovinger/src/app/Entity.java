...
public class Entity {
	...
	public Entity(String[][] b, Player p, List<Tile> gameArr, List<Entity> entityArr, List<String> infoArr, List<Item> itemArr) {
		this.board = b;
		this.p = p;
	...
	public int candy() { return this.candy; }
	public void drop(Item item) {
		Item i = new Item(item); 
		i.setPos(this.x, this.y);
		itemArr.add(i);
	}
	public boolean hasDrops() { return this.inventory.size() > 0; }
	public void addDropTable(Item item) { this.inventory.add(item); }
	...
	public void batAI() {
		...
		for(int i = 0; i < rndLoop; i++) {
			...
			if (rnd == 1) { moveX(1); }
			else if (rnd == 2) { moveX(-1); }
			else if (rnd == 3) { moveY(1); }
			else if (rnd == 4) { moveY(-1); }
		}
	}
	public void slimeAI() {
		for(int i = 0; i < 2; i++) {
			...
			if (!wlked) {
				if (this.x < p.x) { moveX(1); }
				else if (this.x > p.x) { moveX(-1); }
				else if (this.y < p.y) { moveY(1); }
				else if (this.y > p.y) { moveY(-1); }
			}
		}
	}
	...
	public void modifyBuff(Buff b) { this.buff = b; }
	public void attackTile(int x, int y) {
		int dmg = Math.max(0, (int)((this.stats.get("atk")+this.bstats.get("atk")) - (p.stats.get("def")+p.bstats.get("def"))));
