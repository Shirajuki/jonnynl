...
public class Player extends Entity {
	...
	private boolean checkIsEntityAndAtk(int x, int y) {
		for (int i = entityArr.size()-1; i >= 0; i--) {
			...
			if (e.alive) {				
				if (e.x == x && e.y == y) {
					int dmg = Math.max(0, (int)((this.stats.get("atk")+this.bstats.get("atk")) - (e.stats.get("def")+e.bstats.get("def"))));
	...
	public List<String> status() {
		...
		String hp   = " ".repeat(9-(getStringStat("hp")+"/"+getStringTotalStat("hpMax")).length())+(getStringStat("hp")+"/"+getStringStat("hpMax"));
		...
		String atk  = " ".repeat(9-(getStringStat("atk")+getStringBonusStat("atk")).length())+getStringStat("atk")+getStringBonusStat("atk");
		String def  = " ".repeat(9-(getStringStat("def")+getStringBonusStat("def")).length())+getStringStat("def")+getStringBonusStat("def");
		String lck  = " ".repeat(9-(getStringStat("lck")+getStringBonusStat("lck")).length())+getStringStat("lck")+getStringBonusStat("lck");
	...
	public void receiveCandy(int amt) { this.candy += amt; }
	public boolean receiveExp(double exp) {
		this.addExp(exp);
		//System.out.println(this.stats.get("exp")+" [EXP] "+(nextLevel(this.stats.get("lvl").intValue())));
	...
	public void modifyATK(int amt) { this.stats.put("atk", this.stats.get("atk")+amt); infoArr.add("You look a little stronger.");}
	public void modifyDEF(int amt) { this.stats.put("def", this.stats.get("def")+amt); infoArr.add("You look a little tougher.");}
	public void modifyLCK(int amt) { this.stats.put("lck", this.stats.get("lck")+amt); infoArr.add("You feel a little luckier.");}
	...
	public String getDeathMessage() { return this.deathMsg; }
	public void setDeathMessage(String msg) { this.deathMsg = msg; }
	public String getDeathInfo() { 
		String s = "Lasted "+this.steps+" turns and "+this.floor+" floors. Killed "+this.killCount+" monsters.";
		return s;
	}
	@Override
	public void addExp(double amt) { this.stats.put("exp", this.stats.get("exp")+amt); }
	public void killedEnemy() { this.killCount++; }
	@Override
	public void attacked(int atk) {
		Double d = this.stats.get("hp");
		d -= atk;
		this.stats.put("hp", d);
		if (this.stats.get("hp") <= 0) this.alive = false;
	}
	@Override
	...
	public void setPlayerRadius() {
		...
		for (int y = board.length-1; y >= 0; y--) {
			for (int x = board[y].length; x >= 0; x--) {
        		if (Math.pow(this.x-x,2) + Math.pow(this.y-y,2) <= Math.pow(getTotalStat("viewRange"), 2) && Math.pow(this.x-x,2) + Math.pow(this.y-y,2) > Math.pow(getTotalStat("viewRange"), 2)*0.6) {
        			radius.add(x+"-"+y);
        		}    			
            }
        }
