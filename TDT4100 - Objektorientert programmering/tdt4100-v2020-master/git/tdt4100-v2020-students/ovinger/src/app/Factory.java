...
public class Factory {
	...
	public Factory(String[][] b, Player p, List<Tile> gameArr, List<Entity> entityArr, List<String> infoArr, List<Item> itemArr){
		this.board = b;
		this.p = p;
	...
	public String openSpot() {
		...
		for (int y = rndY; y < board.length; y++) {
			for (int x = rndX; x < board[0].length; x++) {
				if (board[rndY][rndX].equals(".")) return rndX+"-"+rndY;
				rndX = x;
				rndY = y;
			}
		}
	...
	public void openSpotItem(Item i) {
		...
		int y = Integer.parseInt(splitted[1]);
		i.setPos(x, y);
		if (i.glyph().equals("<")) System.out.println(x+"-STAIRS-"+y);
	}
	// MONSTERS
