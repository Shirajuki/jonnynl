...
public class Game {
	...
	public void gameLoop() {
		...
		if (!p.alive()) {
			...
			lb.add(formatter.format(new Date())+","+p.name+","+p.getDeathMessage()+","+p.floor+","+p.getStringStat("lvl")+","+p.candy);
	...
	public void newInputScreen(int n) {
		...
		if (screen.screenState.equals("menu")) {
			...
			if (n >= inventoryShown.size()) return;
	...
	public void spawnMonsters() {
		for (int i = 0; i < 10; i++) { factory.newRandomSword(); }
		for (int i = 0; i < 10; i++) { factory.newRandomArmor(); }
		for (int i = 0; i < 25; i++) { factory.newRandomFood(); }
		for (int i = 0; i < 16; i++) {			
		...
		for (int y = board.length-1; y >= 0; y--) {
			for (int x = board[y].length-1; x >= 0; x--) {
				...
				if (p.x == x && p.y == y) {
					...
					if (isItem(x,y)) infoArr.add("This is a "+itemArr.get(foundItemIndex).name());
				} else {
					if (isOnSurface) {
						color = Color.WHITE;
						glyph = board[y][x];
						...
						t.updateTilePos(x*TILE_SIZE-(p.x*TILE_SIZE)+GAME_WIDTH/2-2*TILE_SIZE, y*TILE_SIZE-(p.y*TILE_SIZE)+GAME_HEIGHT/2+TILE_SIZE,Pos.CENTER);
						continue;
					}
					if (p.inRadius(x, y)) {
						glyph = board[y][x];
						if (glyph.equals(".")) {
							...
							if (!tt.visited()) tt.setVisited(true);
						}
						// BRESENHAM LINE
						...
						while (true) {
							if (board[yy][xx].equals("#")) {
								...
								if (!t.visited()) t.setVisited(true);
								break;
							} else {
								color = Color.WHITE;
				...
				Tile t = (Tile)tileArr.get(y).get(x);
				if (t.visited() && !glyph.equals(board[y][x]) && !(x == p.x && y == p.y) && !isEntity(x,y) && !(isItem(x,y))) {
	...
	public boolean isEntity(int x, int y) {
		for (int i = entityArr.size()-1; i >= 0; i--) {
			...
			if (e.alive) {				
				...
				if (e.hasDrops()) e.dropItemFromDropTable();
				entityArr.remove(i);
			}
		}
		return false;
